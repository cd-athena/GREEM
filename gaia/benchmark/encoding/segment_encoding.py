...
def create_ffmpeg_encoding_command(
    ...
    cmd.extend(get_representation_ffmpeg_flags([rendition], dto.preset, dto.codec, fps=fps_str))
        
    fps: int = ceil(VideoInfo(input_file_path).get_fps()) if dto.framerate is None or dto.framerate == 0 else dto.framerate
    keyframe: int = fps * 4
