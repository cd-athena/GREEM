...
representing resolutions and representations, and generating encoding and decoding DTOs.
...
Classes:
    ...
    Representation: Pydantic BaseModel representing a video representation, inheriting from Resolution.
...
class Representation(Resolution):
    """Represents a representation of a video file

    Example:
    `Representation(1080, 1920, 8100)` a representation with a height of 1080,
    a width of 1920 and a bitrate of 8100 kilobyte
    """

    bitrate: int

    def get_representation_dir_string(self) -> str:
        """Creates a string representing the directory path of the instance.

        Example:
        `Representation(1080, 1920, 8100)` is represented as a String `8100k_1920x1080`

        Returns
        -------
        str
            Returns the `Representation` instance as a string representing the directory path
        """
        return f"{self.bitrate}k_{self.get_resolution_dir_representation()}"

    @classmethod
    def from_dir_representation(
        cls: Type["Representation"], dir_repr: str
    ) -> "Representation":
        """
        Creates a `Representation` object from a directory representation string.
        ...
        Returns:
            `Representation`: An instance of Representation created from the directory representation.

        Examples:
            >>> Representation.from_dir_representation('800k_1920x1080')
            Representation(bitrate=800, height=1080, width=1920)

            >>> Representation.from_dir_representation('/path/to/800k_1920x1080')
            Representation(bitrate=800, height=1080, width=1920)
        """
        if "/" in dir_repr:
        ...
        return cls(bitrate=int(bitrate), height=int(height), width=int(width))

    @classmethod
    def new(cls: Type["Representation"]) -> "Representation":
        """Returns a new Representation instance with all values set to zero

        Returns
        -------
        Representation
            Representation(bitrate=0, height=0, width=0)
        """
        return cls(bitrate=int(0), height=int(0), width=int(0))
...
class EncodingConfigDTO(BaseModel):
    ...
    Attributes:
        ...
        representation(Representation): The representation to be used during encoding.
    ...
    def get_output_directory(
        ...
        output_dir: str = f"{self.codec}/{self.preset}/{self.representation.get_representation_dir_string()}"
...
class EncodingConfig(BaseModel):
    ...
    Attributes:
        ...
        representations(list[Representation]): List of representations to be used during encoding.
    ...
    @classmethod
    def from_file(cls: Type["EncodingConfig"], file_path: str) -> "EncodingConfig":
        """Creates an EncodingConfig object from a YAML file"""
    ...
    def get_all_result_directories(self) -> list[str]:
        ...
        directory_list: list[str] = [
            dto.get_output_directory() for dto in self.get_encoding_dtos()
        ]
    ...
    def get_encoding_dtos(self) -> list[EncodingConfigDTO]:
        ...
        for duration, preset, representation, codec, fr in itertools.product(
            segment_duration,
            self.presets,
...
class DecodingConfigDTO:
    ...
    Attributes:
        ...
        encoding_representation(Representation): A filter to only decode videos encoded with a given video representation.
    """
    ...
    encoding_codec: str
    encoding_preset: str
    encoding_representation: Representation
    ...
    def get_output_dir(self, result_dir: str, video_name: str) -> str:
        ...
        output_sub_folders: list[str] = [
            ...
            self.encoding_representation.get_representation_dir_string(),
...
class DecodingConfig(BaseModel):
    ...
    Attributes:
        ...
        encoding_representations(list[Representation]): List of representations to be used during encoding.
    """
    ...
    encoding_representations: list[Representation]
    ...
    def get_decoding_dtos(self) -> list[DecodingConfigDTO]:
        ...
        for (
            ...
            self.encoding_preset,
            self.encoding_representations,
        ):
            dto = DecodingConfigDTO(
                scale_res, fps, enc_codec, enc_preset, enc_representation
            )
