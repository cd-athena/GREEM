...
def create_simple_multi_video_ffmpeg_command(
    ...
    for idx in range(len(video_input_file_paths)):
        ...
        cmd.extend([f"{output_directories[idx]}/output.mp4"])
...
def create_multi_video_ffmpeg_command(
    ...
    for idx in range(len(video_input_file_paths)):
        ...
        cmd.extend(map_cmd)
        cmd.extend([f"{output_directories[idx]}/{output_file_names[idx]}.mp4"])
...
def create_multi_video_ffmpeg_yuv_to_mp4_command(
    ...
    if cuda_mode and gpu_count > 0:
        cmd.extend(
            [
                f"-f rawvideo -video_size 3840x2160 -pix_fmt yuv420p -hwaccel_device {idx % gpu_count} {CUDA_ENC_FLAG} -i {video}"
                for idx, video in enumerate(video_input_file_paths)
            ]
        )
        # cmd.extend(
        ...
        cmd.extend(
            [
                f"-f rawvideo -video_size 3840x2160 -pix_fmt yuv420p -i {video}"
                for video in video_input_file_paths
            ]
        )
...
def multi_video_ffmpeg_yuv_to_mp4_command_per_gpu(
    ...
    cmd.extend(
        [
            f"-hwaccel_device {gpu_idx} {CUDA_ENC_FLAG} -f rawvideo -video_size 3840x2160 -pix_fmt yuv420p -t 5 -i {video}"
            for video in video_input_file_paths
        ]
    )
...
def get_slice_video_command(
    ...
    duration: str = (
        f"{segment_duration}" if segment_duration > 9 else f"0{segment_duration}"
    )
...
def prepare_sliced_videos(
    ...
    for config in encoding_configs:
        videos.update(
            [get_video_without_extension(file) for file in os.listdir(input_dir)]
        )
    ...
    input_files: list[str] = [
        ...
        if get_video_without_extension(file) in videos
    ]
    ...
    for duration in durations:
        for input_file, output_file in zip(input_files, output_files):
            cmd_list = get_slice_video_commands(
                f"{input_dir}/{input_file}", output_dir, output_file, duration
            )
...
class CodecProcessing(BaseModel):
    ...
    def create_sequential_encoding_cmd(
        ...
        if dto.codec in ["avc", "h264", "hevc", "h265"]:
            cmd.extend(
                self.h26x_sequential_encoding_cmd(
    ...
    def h26x_sequential_encoding_cmd(
        self, input_file_path: str, dto: EncodingConfigDTO, constant_rate_factor: int
    ) -> list[str]:
        cmd: list[str] = []
