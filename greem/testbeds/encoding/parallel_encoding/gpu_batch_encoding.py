...
from greem.utility.timing import (
    TimingMetadata,
    measure_time_of_system_cmd,
    IdleTimeEnergyMeasurement,
)
...
def prepare_data_directories(
    ...
    if encoding_variant == EncodingVariant.BATCH:
        data_directories = [
            "/".join(data_dir.split("/")[:-1]) for data_dir in data_directories
        ]
...
def get_video_input_files(video_dir: str, encoding_config: EncodingConfig) -> list[str]:
    input_files: list[str] = [
        ...
        or file_name.split(".")[0] in encoding_config.videos_to_encode
    ]
...
def prepare_all_video_directories(
    encoding_configs: list[EncodingConfig], encoding_variant: EncodingVariant
) -> None:
    for encoding_config in encoding_configs:
